...
// Nuevo: referencia a Portón delantero y control dinámico del botón principal
...
// Pivot para portón trasero (para bisagra superior)
let portonTraseroPivot = null;
// Pivot para portón delantero (bisagra superior)
...
// X bloqueado para la cortina trasera
const REAR_LOCKED_X = -0.710;
// Z bloqueado para la cortina trasera
const REAR_LOCKED_Z = 0.098;
// Y bloqueado para la cortina trasera
const REAR_LOCKED_Y = 0.585;

// Utilidad: obtener tamaño aproximado de la puerta de garaje para escalar paneles
...
// Crear panel rojo con pivot superior (para crecer solo hacia abajo)
...
// Drag & drop simple en pantalla (mueve pivots en plano Y/Z)
let dragState = { active: false, target: null, offset: new THREE.Vector3(), mode: 'xy', pointerId: null };
...
function enableDragForPivot(pivot) {
	...
	renderer.domElement.addEventListener('pointermove', (e) => {
		...
		if (dragState.mode === 'z') {
			...
			// Solo Z con click derecho
			if (pivot === cortinaTraseraPanelPivot) {
				pivot.position.z = REAR_LOCKED_Z; // bloquear Z
				pivot.position.x = REAR_LOCKED_X; // mantener X bloqueado también
				pivot.position.y = REAR_LOCKED_Y; // bloquear Y
			} else {
			...
			if (!world) return;
			const next = world.clone().add(dragState.offset);
			// Mover en X e Y (mantener Z constante del pivot)
			if (pivot === cortinaTraseraPanelPivot) {
				pivot.position.x = REAR_LOCKED_X; // bloquear X
				pivot.position.z = REAR_LOCKED_Z; // bloquear Z
				pivot.position.y = REAR_LOCKED_Y; // bloquear Y
			} else {
...
// 🧠 Función robusta sin vibración para rotación
...
// 🧠 Función para animar solo rotación (para el garaje)
...
function setupLightButtons(root) {
  ...
  root.traverse((child) => {
    if (child.isMesh && isLightLikeName(child.name)) {
      // Aislar materiales para que los cambios no afecten a otras mallas que compartan material
      cloneMeshMaterials(child);
      // Guardar estado original de sus materiales clonados
